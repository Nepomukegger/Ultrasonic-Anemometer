...
#let thesis-titlepage() = context {
  ...
  // Autoren und Matrikelnummern als Array normalisieren
  let authors = normalize-multivalue(p("author"))
  let student-ids = normalize-multivalue(p("student-id", default: none))
  ...
  // Über die Autoren iterieren und anzeigen
  if authors != none {
    for (i, author) in authors.enumerate() {
      text(size: 14.4pt, author)
      if student-ids != none and student-ids.len() > i {
        [\ #t("student-id"): #student-ids.at(i)]
      }
      if i < authors.len() - 1 {
        v(3 * ex) // Abstand zwischen mehreren Autoren
      }
    }
  }
  v(8 * ex)
  ...
  text(size: 8pt)[
    ...
    // Copyright auf mehrere Autoren anpassen
    #let copyright = if company != none { company } else if authors != none { authors.join(", ") } else { "" }
